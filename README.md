...
The agent has a fixed **1-minute generation benchmark** per experiment. It modifies `infer.py` and tries to maximize **output tokens per second (`tokens_per_sec`)**, subject to a bits-per-byte quality guard.
...
**Quality guard: `bpb` (must not degrade)** — bits per byte on a fixed WikiText-2 test set, evaluated after warmup. Must stay within 1% of baseline to catch optimizations that trade quality for speed.

## Scope

The agent may only modify `infer.py`. All other files are read-only.

The agent may not modify model weights, install new packages, or tune hyperparameters.
